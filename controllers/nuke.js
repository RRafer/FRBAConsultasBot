			...
			bot.getChatMember(msg.chat.id, msg.from.id).then(userMember => {
				...
				idToBan.forEach((id) => {
					...
					bot.getChatMember(msg.chat.id, id).then((userToBan) => {
						...
						bot.kickChatMember(chatGroupId, id).then(() => {
							if (chatGroupId == msg.chat.id)
								bot.sendMessage(chatGroupId, `${generateMention(msg)} ha banneado a ${mentionToBan} !`, { parse_mode: 'Markdown' }).catch(err => { logger.error(`Error sending message after banning user: ${err}`);}); // Catch weird errors?
						}).catch(err => { logger.error(`Error banning user: ${err}`);}); // Catch 'cannot ban user' errors
